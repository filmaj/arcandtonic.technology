export default class Tonic {
    ...
    static add (c) {
        ...
        if (!c.name || c.name.length === 1) {
            throw Error(`Mangling detected, see guide. https://github.com/hxoht/tonic/blob/master/HELP.md.`);
        }
        ...
        if (c.registered) {
            throw new Error(`Already registered ${c.name}`);
        }
        c.registered = true;
    ...
    reRender (o = this.props) {
        ...
        if (!this.root) {
            throw new Error(`.reRender called on destroyed component, see guide.`);
        }
